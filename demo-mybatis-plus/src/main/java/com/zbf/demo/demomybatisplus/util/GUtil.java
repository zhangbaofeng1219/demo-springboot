package com.zbf.demo.demomybatisplus.util;
...
//这里按着给的注解修改参数即可
        ...
        //手动配置数据源
        String url="jdbc:mysql://localhost:3306/demo-springboot?serverTimezone=UTC&useUnicode=true&characterEncoding=utf-8";   //注意修改数据库名
        String name="root";
        String password="root";

        //数据库表的设置
        List<String> listTable = Arrays.asList("demo_table_01");  //设置需要自动代码生成的表名
        ...
        FastAutoGenerator.create(url, name, password)
                ...
                .globalConfig(builder -> {
                    builder.author(author) // 设置作者名
                            .outputDir(System.getProperty("user.dir") + "/demo-mybatis-plus/src/main/java")   //设置输出路径：项目的 java 目录下【System.getProperty("user.dir")意思是获取到项目所在的绝对路径】
                ...
                .packageConfig(builder -> {
                    builder.parent(parent) // 设置父包名
                            ...
                            .pathInfo(Collections.singletonMap(OutputFile.mapper.xml, System.getProperty("user.dir")+"/demo-mybatis-plus/src/main/resources/mapper"));    //配置 mapper.xml 路径信息：项目的 resources 目录下
                })
                //4、策略配置
